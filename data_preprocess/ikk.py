...
def create_ikk():
    ...
    """创建一个np数组，形状为（i，k，k）"""
    ikk_np = np.zeros((len(merge_df['航班ID'].values.tolist()),len(merge_df['航班ID'].values.tolist()),len(airport_df['到达机场'].values.tolist())))
    ...
    """航班id重复i词"""
    index2 = merge_df['航班ID'].values.tolist() * len(merge_df['航班ID'].values.tolist())
    print (ikk_np)
    ikk_np = ikk_np.reshape(-1,5)
    """构建层次化索引"""
    index = pd.MultiIndex.from_arrays([index1,index2],names=['航班ID',"起飞机场ID"])
    """创建dataframe并写入文件"""
    ikk_df = pd.DataFrame(ikk_np,index=index,columns=range(1,len(airport_df['到达机场'].values.tolist())+1))
